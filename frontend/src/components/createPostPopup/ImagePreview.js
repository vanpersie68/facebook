...
export default function ImagePreview({ text, user, setText, images, setImages, setShowPrev, setError }) {
    ...
    const handleImages = (event) => {
        ...
        files.forEach((img) => {
            ...
            if(img.type !== "image/jpeg" && img.type !== "image/png" && img.type !== "image/webp" && img.type !== "image/gif"){
                ...
                files = files.filter((item) => item.name != img.name); //移除不符合要求的图片
                return;
                ...
                {images && images.length ? (
                    <div className="add_pics_inside1 p0">
                        ...
                        <div className={images.length === 1 ? "preview1" : 
                            ...
                            images.length % 2 === 0 ? "preview6" : "preview6 singular_grid"
                        }>
                            {
                                images.map((img, i) => (
