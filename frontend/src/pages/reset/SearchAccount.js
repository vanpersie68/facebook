...
export default function SearchAccount({ email, setEmail, error, setError, setLoading, setUserInfos, setVisible }) {
    ...
    const handleSearch = async() => {
        try{
            ...
            const {data} = await axios.post(
                `${process.env.REACT_APP_BACKEND_URL}/findUser`, {email}
            );
    ...
    return (
        <div className="reset_form" >
            <div className="reset_from_header">Find Your Account</div>
                ...
                {(formik) => (
                    <Form>
                        <LoginInput 
                            type="text"
                            name="email"
                            placeholder="Email address or phont number"
                            onChange={(event) => setEmail(event.target.value)}
                        />
                        {error && <div className="error_text">{error}</div>}
                        <div className="reset_form_btns">
                            <Link to="/login" className="gray_btn" >Cancel</Link>
                            <button type="submit" className="blut_btn">Search</button>
                        </div>
                    </Form>
                )}
            </Formik>
